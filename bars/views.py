...
def search_castimage_files(prefix):
    
    media_root = os.path.abspath(settings.MEDIA_ROOT)

    # Loop over all files in the media directory
    filenames = os.listdir(media_root + '/casts/')
    matching_filenames = []
    for filename in filenames:
        # Check if the file has the given prefix and is an image file
        if filename.startswith(prefix) and filename.lower().endswith(('.jpg', '.jpeg', '.png')):
...
def casts_checkcastimage_exist(request):
    ...
    filenames = search_castimage_files(prefix)
...
@csrf_exempt
def set_currentStoreId(request):

    # ======== analysis request ==============.
        data = json.loads(request.body)
        store_id = data['id']

        sql_select = "select * from bars_temp_bar_cast where id = 1"
        sql_update = "update bars_temp_bar_cast set barid = {0} where id = 1 ".format(store_id)
        ...
        with connection.cursor() as cursor: 
            ...
            if len(results) > 0:
                ...
                sql_insert = "insert into bars_temp_bar_cast(id, barid, castid) value(1, {0}, null)".format(store_id)
                cursor.execute(sql_insert)

        # return the JSON response
        return JsonResponse({'status': 'success'})

def get_currentStoreId(request):

    # ======== analysis request ==============.
        
        sql_select = "select barid from bars_temp_bar_cast where id = 1"
        
        with connection.cursor() as cursor: 

            cursor.execute(sql_select)
            barId = cursor.fetchone()[0]

            response_data = {'barId': barId}
        # return the JSON response
        return JsonResponse(response_data)

def get_currentStoreRooms(request):

    # ======== analysis request ==============.
    key = handleRequest(request)

    sql = """SELECT bbr.bar_room_label, bbr.bar_room_name, bbr.bar_room_description 
            FROM bars_bar_fk_rooms bbk 
            left join bars_bar_room bbr on bbk.bar_room_id = bbr.id where bar_id = {0}""".format(key)
